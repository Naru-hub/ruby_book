...
# selfなしでnameメソッドを呼び出す場合、self付きで呼び出す場合、直接インスタンス変数にアクセスする場合
class User
  attr_accessor :name
...
class User
  ...
  def rename_to_bob
    #selfなしでname=メソッドを呼ぶ(?) nemeというローカル変数に"Bob"という文字列に代入したと解釈される 下のようにself.をつける
    name = 'Bob'
  end
...
puts product.to_s     #"name: A grat movie, price: 1000円"